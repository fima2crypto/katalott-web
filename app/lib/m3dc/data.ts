...
import { M3DCKq, M3DCKqRow } from '@/app/lib/m3dc/definitions';
...
// ==================== COMPUTE ====================
function computeM3DCRow(row: M3DCKq, prevN20: string[]): M3DCKqRow {
...
// ==================== FETCH ====================
export async function fetchM3DCKqList(params: {
  tu_ky?: string;
  den_ky?: string;
  limit?: number;
}): Promise<M3DCKqRow[]> {
  ...
  if (tu_ky && den_ky) {
    rows = await sql<M3DCKq[]>`
      SELECT * FROM public.m3dckq
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    result.push(computeM3DCRow(rows[i], prevN20));
  }
  return result;
}
