...
import { fetchM3DCKqList } from "@/app/lib/m3dc/data";
import { M3DCKqRow } from "@/app/lib/m3dc/definitions";
import M3DCFilter from "@/app/(products)/m3dc/ui/M3DCFilter";
...
// ==================== ROW ====================
function M3DCRow({ row, idx }: { row: M3DCKqRow; idx: number }) {
  const db1 = row.db[0] ?? "";
  const db2 = row.db[1] ?? "";
  ...
  const thuBg =
    thuTrim === "T2" ? "bg-orange-400" :
    thuTrim === "T4" ? "bg-blue-400" :
    thuTrim === "T6" ? "bg-green-400" :
    "";
...
// ==================== TABLE ====================
function M3DCTable({ data }: { data: M3DCKqRow[] }) {
  if (!data.length)
    return (
  ...
  return (
    <div className="overflow-auto">
      <table className="border-collapse text-xs min-w-max">
        ...
        <tbody>
          {data.map((row, idx) => (
            <M3DCRow key={row.ky} row={row} idx={idx + 1} />
          ))}
        </tbody>
      </table>
    </div>
...
// ==================== MAIN ====================
export default async function M3DCKqPage({
  searchParams,
}: {
  ...
  const data = await fetchM3DCKqList(
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-mono">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center gap-4 flex-wrap">
        ...
        <h1 className="text-lg font-bold text-blue-800">🎯 Max 3D+ </h1>
        <M3DCFilter mode={mode} tuKy={tu_ky} denKy={den_ky} limit={limit} />
      </div>
